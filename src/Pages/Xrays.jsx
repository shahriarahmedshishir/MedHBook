...
const Xrays = () => {
  ...
  const handleRemoveStoredFile = async (id) => {
    try {
      ...
      console.error(err);
      alert("Error deleting xrays: " + err.message);
    }
  };
  ...
  const handleDownloadSelected = async () => {
    for (const id of selectedForDelete) {
      ...
      const fileName = `Xrays${item.doctorName || "Unknown"}.jpg`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSelectedFiles([]);
      setDoctorName("");
      alert("Xray(s) uploaded successfully!");
    } catch (err) {
      console.error(err);
      alert("Error uploading Xrays: " + err.message);
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-3xl font-bold mb-6">My Xray's</h1>
